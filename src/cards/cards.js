...
const cardElements =
	page() === 'Teachers' ? cardElementsTeachers : cardElementsEvents
	...
	if (deviceType() === 'desktop') {
		cards[i].addEventListener('mouseover', () =>
			cardElements.forEach(element =>
				element[i] ? element[i].classList.add('hover-tap') : null
			)
		)
		cards[i].addEventListener('mouseout', () =>
			cardElements.forEach(element =>
				element[i] ? element[i].classList.remove('hover-tap') : null
			)
		)
	}
	// ...mobile and tablet use `tap` response.
		...
		cards[i].addEventListener('click', () => {
			...
			for (let j = 0; j < cards.length; j++) {
				if (i !== j) {
					cardElements.forEach(element =>
						element[j] ? element[j].classList.remove('hover-tap') : null
					)
				}
			}
		})
