...
const formValidation = () => {
	if (
		!isBlank(customFirstName) &&
		!isBlank(customLastName) &&
		!isBlank(customEmail) &&
		!isBlank(customMobile) &&
		getValue(customSelect) &&
		isChecked(customTerms) &&
		billingValidation()
	) {
		emptyText(getElementById('billing-card-error'))
		setCss(paymentButton, {
			property: 'style',
			value: 'background-color: #800000'
		})
...
const showErrorsInForm = () => {
	if (!isChecked(customTerms)) {
		setCss(customTermsValidation, {
			property: 'style',
			value: 'background-color: #fdd'
		})
	} else {
		setCss(customTermsValidation, {
			property: 'style',
